...
const EditCefLayout = ({ id, onClose }) => {
  // ex data
  const titleEx =
  ...
  useEffect(() => {
    const fetchData = async () => {
      const cefTemplate = await getCefTemplateById(id);
      setEventTitle(cefTemplate.data.EventTitle);
      setCefHost(cefTemplate.data.CefHost);
      setStudentName(cefTemplate.data.StudentName);
      setStudentCode(cefTemplate.data.StudentCode);
      setCefNo(cefTemplate.data.CefNo);
      setCefDay(cefTemplate.data.CefDay);
      setCefMonth(cefTemplate.data.CefMonth);
      setCefYear(cefTemplate.data.CefYear);
      setIsDefault(cefTemplate.isDefault);
      setCefTemplateTitle(cefTemplate.name);
    };
    fetchData();
  }, [imageSize]);
  ...
  const handleUpdateCefLayout = async () => {
    try {
      if (isDefault) {
      ...
      await updateCefTemplate(id, cefTemplate);
      onClose();
    } catch (error) {
      setError(error);
    }
